...
class Pedido(Base):
    ...
    numero_pedido: Mapped[str | None] = mapped_column(String(50), unique=True, index=True, nullable=True)
    cliente_id: Mapped[int | None] = mapped_column(ForeignKey("clientes.id"), nullable=True) # Now optional for new ad-hoc clients
    nit_cedula: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    iva: Mapped[Decimal] = mapped_column(Numeric(12, 2), nullable=False, default=0.0)
    descuento: Mapped[Decimal] = mapped_column(Numeric(12, 2), nullable=False, default=0.0)
    ...
    condiciones_entrega: Mapped[str | None] = mapped_column(String(250), nullable=True)
    fecha_estimada_entrega: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
