...
const App = {
    ...
    async syncWithBackend() {
        ...
        try {
            ...
            // 1.6 Sync any pending offline abonos
            if (this.state.unsyncedAbonos && this.state.unsyncedAbonos.length > 0) {
                this.showToast(`Sincronizando ${this.state.unsyncedAbonos.length} recaudo(s)...`);
                const syncResult = await ApiClient.syncAbonos(this.state.unsyncedAbonos);
                
                if (syncResult.total_insertados > 0 || syncResult.total_duplicados > 0) {
                    this.state.unsyncedAbonos = [];
                    localStorage.setItem(this.getDbPrefix() + 'sam_unsynced_abonos', JSON.stringify([]));
    ...
    renderDashboard() {
        ...
        if(this.state.unsyncedAbonos) pending += this.state.unsyncedAbonos.length;
        if(this.state.unsyncedClientes) pending += this.state.unsyncedClientes.length;
    ...
    confirmVisitCheckout() {
        ...
        for (let pId in this.state.cart) {
            ...
            const pdist = parseFloat(prod.precio_distribuidor) || 0;
            const pdir = parseFloat(prod.precio_directo) || 0;
            const precioActual = (this.state.tipoCliente === 'distribuidor' && pdist > 0) ? pdist : pdir;
...
,
    ...
    // --- CARTERA LOGIC ---
    renderCarteraView() {
        if (!this.state.activeClient) return;
        const cid = this.state.activeClient.id;
        
        const clientFacturas = (this.state.cartera || []).filter(f => f.cliente_id === cid);
        
        let adeudado = 0;
        let vencido = 0;
        const listEl = document.getElementById('cartera-facturas-list');
        if(!listEl) return;
        listEl.innerHTML = '';
        
        if (clientFacturas.length === 0) {
            listEl.innerHTML = '<p style="color:var(--text-secondary); text-align:center; margin-top:20px;">No hay facturas pendientes.</p>';
        } else {
            clientFacturas.forEach(f => {
                adeudado += parseFloat(f.saldo_pendiente);
                if (f.estado === 'VENCIDA') {
                    vencido += parseFloat(f.saldo_pendiente);
                }
                
                const fDate = new Date(f.fecha_vencimiento).toLocaleDateString();
                const fDiv = document.createElement('div');
                fDiv.style = "padding:12px; border:1px solid var(--border-color); border-radius:var(--border-radius-md); margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;";
                fDiv.innerHTML = `
                    <div>
                        <div style="font-weight:600; font-size:0.95rem;">${this.escapeHtml(f.numero_factura)} <span style="font-size:0.75rem; padding:2px 6px; border-radius:10px; background:${f.estado==='VENCIDA'?'#fee2e2':'#dcfce3'}; color:${f.estado==='VENCIDA'?'#b91c1c':'#166534'}; margin-left:4px;">${f.estado}</span></div>
                        <div style="font-size:0.8rem; color:var(--text-secondary);">Vence: ${fDate}</div>
                    </div>
                    <div style="font-weight:bold; color:var(--text-primary); text-align:right;">
                        $${parseFloat(f.saldo_pendiente).toLocaleString('es-CO')}
                    </div>
                `;
        ...
        const totAd = document.getElementById('cartera-total-adeudado');
        if(totAd) totAd.innerText = `$${adeudado.toLocaleString('es-CO')}`;
        const totV = document.getElementById('cartera-total-vencido');
        if(totV) totV.innerText = `$${vencido.toLocaleString('es-CO')}`;
    },
    ...
    showAbonoModal() {
        ...
        if (!this.state.activeClient) return;
        const cid = this.state.activeClient.id;
        const clientFacturas = (this.state.cartera || []).filter(f => f.cliente_id === cid && parseFloat(f.saldo_pendiente) > 0);
        ...
        if (clientFacturas.length === 0) {
            this.showToast('No hay facturas con saldo pendiente para abonar.', true);
            return;
        }
        ...
        clientFacturas.forEach(f => {
            ...
            opt.value = f.id;
            opt.text = `${f.numero_factura} (Saldo: $${parseFloat(f.saldo_pendiente).toLocaleString('es-CO')})`;
            // store max amount
            opt.dataset.max = f.saldo_pendiente;
    ...
    handleSaveAbono(e) {
        ...
        if (!fId || isNaN(monto) || monto <= 0) {
            this.showToast("Datos inválidos", true);
            return;
        }
        
        const opt = select.options[select.selectedIndex];
        const maxMonto = parseFloat(opt.dataset.max);
        
        if (monto > maxMonto) {
            this.showToast(`El monto no puede superar el saldo pendiente ($${maxMonto.toLocaleString('es-CO')})`, true);
            return;
        }
        ...
        if (!this.state.unsyncedAbonos) this.state.unsyncedAbonos = [];
        this.state.unsyncedAbonos.push(newAbono);
        localStorage.setItem(this.getDbPrefix() + 'sam_unsynced_abonos', JSON.stringify(this.state.unsyncedAbonos));
        
        // Update local cartera state immediately
        const fIndex = this.state.cartera.findIndex(f => f.id === fId);
        if (fIndex !== -1) {
            this.state.cartera[fIndex].saldo_pendiente = parseFloat(this.state.cartera[fIndex].saldo_pendiente) - monto;
            if (this.state.cartera[fIndex].saldo_pendiente <= 0) {
                this.state.cartera[fIndex].estado = 'PAGADA';
            }
            localStorage.setItem(this.getDbPrefix() + 'sam_cartera', JSON.stringify(this.state.cartera));
        }
        
        this.showToast("Abono registrado localmente");
        this.closeAbonoModal();
        this.renderCarteraView();
        
        this.updateSyncBadge();
    },

    async handleUploadCarteraExcel(event) {
        const file = event.target.files[0];
        if (!file) return;
        this.showToast("Subiendo cartera de Siigo...");
        try {
            const result = await ApiClient.uploadAdminCarteraExcel(file);
            this.showToast(`Éxito: ${result.nuevas_facturas} nuevas facturas procesadas.`);
            // Refresh admin dashboard stats if we had any
